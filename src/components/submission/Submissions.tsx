...
export default function Submissions({ projectId }: { projectId: string }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const submissions = await submissionService.getProjectSubmission(
          orgId.toString() || '',
          projectId,
          token
        );
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className='uppercase'>Form By</TableHead>
              ...
              <TableBody>
                {rows.map((row, index) => (
                  <TableRow key={index}>
                    <TableCell>
                      <Link
                        href={`/orgs/${orgId}/projects/${projectId}/submissions/${row.submissionId}`}
                      >
                        {row.title}
                      </Link>
                    </TableCell>
                    ...
                    <TableCell className='text-muted-foreground text-right'>
                      <Badge title='View Submission' variant={'outline'}>
                        <Link
                          href={`/orgs/${orgId}/projects/${projectId}/submissions/${row.submissionId}`}
                        >
                          {<LuView className='text-blue-600 w-10 h-5' />}
                        </Link>
                      </Badge>
                    </TableCell>
                  </TableRow>
